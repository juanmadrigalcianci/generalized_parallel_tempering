...
sigma_is=np.array([[0.1],[0.2], [0.4],[.8]]);#*linspace(1,100,N_temp);
...
for i in range(Nruns):
    ...
    Zptf,Xptf=base.pcn_full_vanilla(post,N,beta,sigma_is,PRIOR,x0,Ns=1,Disp=1,dim_q=dim_q) #full vanilla, reversible
    mean_pf[i,:]=np.mean(Xptf[Burn_in:,:,0],0)
    # _,Xuw=base.pcn_unweighted_IS(post,N,beta,sigma_is,x0,1,1,Disp=0) #unweighted Is
    ...
    # _,Xw,W_IS,W_IS2=base.pcn_weighted_IS(post,N,beta_original,sigma_is,x0,Disp=0) #weighted IS
    ...
    # _,Xrwm=base.pcn(post,N*N_temp,sigma_is[0],x0[:,0],Disp=0) #random walk metropolis
...
name_q='results/est_pt_'+str(INDICATOR).zfill(2)+'.npy'

np.save(name_q,mean_pf)
