...
for i in range(N_temp):
    x0[:,i]=theta_true+0.1*PRIOR.sample(exp=False)
...
for i in range(Nruns):
    ...
    Zuw,Xuw=base.pcn_unweighted_IS(post,N,beta,sigma_is,PRIOR,x0,ts=1,Ns=1,Disp=1,dim_q=dim_q,PLOT=True) #unweighted Is
...
name_q='results/est_uw_'+str(INDICATOR).zfill(2)+'.npy'
name_s='results/samp_uw_'+str(INDICATOR).zfill(2)+'.npy'

np.save(name_q,mean_uw)
np.save(name_s,Zuw)
