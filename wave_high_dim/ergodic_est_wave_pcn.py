...
for i in range(Nruns):
    ...
    Zpcn,Xrwm=base.pcn(post,N,sigma_is[0],PRIOR,z0,Disp=1,dim_q=dim_q) #random walk metropolis
    mean_r[i,:]=np.mean(Xrwm[Burn_in:],0)
    
    
    


name_q='results/est_pcn_'+str(INDICATOR).zfill(2)+'.npy'

np.save(name_q,mean_r)
