...
def parallel_tempering(post, N, beta, sigma_rwm, x0, strategy, Ns=1, Disp=0):
    ...
    #computes each individual posterior
        for k in range(N_temp):
            y[k, :] = X[j, :, k] + sigma_rwm[k] * np.random.standard_normal(
                p, )
            py[j, k] = post(y[k, :])
    ...
    #accepts-rejects
        ...
        if (np.mod(j, Ns) == 0):
            #swaps according to a given strategy
            X, px = swap_strategy(X, px, beta, strategy, j + 1)
...
"""
"""
Creates the functions necesary for the GPT  algorithms,
...
Computes the weights for the weighted GPT
...
Creates the base for weighted GPT
...
def weighted_IS(post, N, beta_original, sigma_rwm, x0, Disp=0):
    ...
    for j in range(N - 1):
        ...
        for k in range(N_temp):
            y[k, :] = X[j, :,
                        k] + sigma_rwm[index[k]] * np.random.standard_normal(
                            p, )
            py[j, k] = post(y[k, :])
